...
@section('content')
    ...
    <script>
        $(document).ready(function() {
            $('#example').DataTable( {
                ajax: "/{{app()->getLocale()}}/doctors/consultations/open/json",
                stateSave:true,
                responsive: true,
                ...
                "columns": [
                    ...
                    { "data": "appointment_selected_by_patient" ,"title":"  {{ __('site.final_appointment') }} ({{Auth::guard('doctor')->user()->timezone->name_en}})"},
                ...
                "columnDefs": [
                    {
                        ...
                        "render": function ( data, type, row ) {
                            if(row.appointment_reply_by_doctor==='no')
                              return "<a href='#' data-id=" + row.id + "  class='btn btn-info btn-sm choose_btn2' >{{ __('site.doctor_reply') }}</a>";
                          else
                            return row.appointment_reply_by_doctor ;
                        },
                        "targets":-7
                    },
                    {
...
{{--    </style>--}}
    <script>
        $(document).ready(function(){
            ...
            $(document).on('click','.choose_btn2',function(e){
                ...
                var panel =      jsPanel.modal.create({
                 ...
                 //   contentSize: "auto",
                    ...
                    headerTitle: '<span style="margin: 0">{{ __('site.create_new_appointments') }}</span>',
                   ...
                   position: 'center 0 -50',
                    ...
                    contentAjax: {
                        ...
                        url :  "{{url(app()->getLocale()."/doctors/appointments/create-five-appointments") }}/"+consultation_id,
            ...
            $(document).on('click','.report_btn',function(e){
                ...
                var panel =      jsPanel.modal.create({
                    ...
                    contentAjax: {
                        method: 'get',
                        url :  "{{url(app()->getLocale()."/doctors/consultations/create-report") }}/"+consultation_id,
                        beforeSend: function() {
                            this.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
                            // this.setRequestHeader('X-CSRF-TOKEN', $('meta[name="csrf-token"]').attr('content'));
                        },
            ...
            $(document).on('click','.show_uploaded_btn',function(e){
                ...
                var panel =      jsPanel.modal.create({
                    ...
                    contentAjax: {
                        method: 'get',
                        url :  "{{url(app()->getLocale()."/doctors/consultations/show-uploaded-files") }}/"+consultation_id,
                        beforeSend: function() {
                            this.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded');
                            // this.setRequestHeader('X-CSRF-TOKEN', $('meta[name="csrf-token"]').attr('content'));
                        },
