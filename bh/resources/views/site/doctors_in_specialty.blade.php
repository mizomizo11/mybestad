...
@section('content')
    <div class="container" style="margin-top: 50px;">
        <div class="row">
            <div class="col-md-12">
                <h1  style="text-align: center;color: #0f9bc0;border-bottom:1px solid #0f9bc0;padding:10px;">
                    {{ __('site.doctors_in_specialty') }} - {{$specialty_name->{"name_".app()->getLocale()} }}
                </h1>
            </div>
        </div>
    </div>
                            ...
                            <div style="padding: 10px">
                                <form  method="post" action="{{url(app()->getLocale()."/patients/steps")}}">
                                    ...
                                    @if(Auth::guard('patient')->check())
                                        <button type="submit"  class="btn btn-sm  btn-info btn-light-primary "  name="login_btn" id="login_btn"   style="width: 200px">{{ __('site.request_a_consultation') }}   </button>
                                    @else
